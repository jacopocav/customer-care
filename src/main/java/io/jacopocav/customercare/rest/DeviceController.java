...
public class DeviceController {
    ...
    @Operation(
        ...
        responses = {
            @ApiResponse(responseCode = "201", description = "Device created successfully",
                headers = @Header(name = "Location", description = "URI of the new device")),
            @ApiResponse(responseCode = "400", description = "Bad request",
                content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
    @PostMapping
    public ResponseEntity<Void> create(
        @RequestBody @Valid CreateDeviceRequest body,
    ...
    public ReadDeviceResponse read(@PathVariable @UUID String id) {
        return crudService.read(id);
    }

    @Operation(
        summary = "Checks whether a device exists or not",
        responses = {
            @ApiResponse(responseCode = "200", description = "The device exists"),
            @ApiResponse(responseCode = "400", description = "Bad request"),
            @ApiResponse(responseCode = "404", description = "The device does not exist"),
        })
    @RequestMapping(method = HEAD, path = "/{id}", consumes = ALL_VALUE)
    public ResponseEntity<Void> exists(@PathVariable @UUID String id) {
        final var exists = crudService.exists(id);
        return ResponseEntity.status(exists ? OK : NOT_FOUND).build();
    }
